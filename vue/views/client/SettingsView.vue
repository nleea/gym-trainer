<template>
  <div class="space-y-5 sm:space-y-6">
    <div>
      <h1 class="text-2xl font-bold text-foreground sm:text-3xl">{{ t('client.settings.title') }}</h1>
    ...
    <div class="rounded-xl border bg-card shadow-sm">
      ...
      <div class="space-y-4 p-4 sm:p-6">
        ...
        <div class="flex justify-end">
          <button
            @click="saveProfile"
            class="w-full rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 sm:w-auto"
          >
    ...
    <RouterLink
      ...
      class="flex items-center gap-3 rounded-xl border bg-card px-4 py-4 shadow-sm transition-colors hover:bg-muted/50 sm:px-6 sm:py-5"
    >
      <div class="min-w-0 flex-1">
        <h2 class="text-lg font-semibold text-foreground sm:text-xl">{{ t('client.settings.appearance') }}</h2>
        <p class="mt-0.5 text-sm text-muted-foreground">{{ t('client.settings.appearanceDesc') }}</p>
      </div>
      <span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-muted/70">
